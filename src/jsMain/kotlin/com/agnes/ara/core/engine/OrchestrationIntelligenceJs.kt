...
import com.agnes.ara.core.domain.service.orchestration.*
...
@JsExport
class AraActionDispatchServiceJs {
    fun canDispatchDeterministically(moduleId: String, actionType: String): Boolean =
        AraActionDispatchService.canDispatchDeterministically(moduleId, actionType)
    fun getDeterministicActions(moduleId: String): Array<String> =
        AraActionDispatchService.getDeterministicActions(moduleId).toTypedArray()
}

@JsExport
class AraDispatchExecutionServiceJs {
    fun detectDependencyCycles(dispatchPlanJson: String): Array<String> =
        AraDispatchExecutionService.detectDependencyCycles(dispatchPlanJson).toTypedArray()

    fun buildExecutionSummary(packetJson: String, resultsJson: String): String =
        AraDispatchExecutionService.buildExecutionSummary(packetJson, resultsJson)

    fun buildRetryPacket(packetJson: String, resultsJson: String): String? =
        AraDispatchExecutionService.buildRetryPacket(packetJson, resultsJson)

    fun buildExecutionId(planId: String, nodeId: String): String =
        AraDispatchExecutionService.buildExecutionId(planId, nodeId)

    fun getRetryParentPlanId(packetJson: String): String? =
        AraDispatchExecutionService.getRetryParentPlanId(packetJson)
    ...
    fun buildExecutionArtifacts(
        ...
        retryPrompt: String?,
    ): String = AraDispatchExecutionService.buildExecutionArtifacts(
