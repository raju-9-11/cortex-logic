...
/**
 * JS-facing bridge for AraSynthesisService.
 *
 * Wraps the KMP [AraSynthesisService] and exposes it to TypeScript via
...
class AraSynthesisServiceJs(
    ...
    private val inner = AraSynthesisService(
        ...
        return buildJsonArray {
            results.forEach { c ->
                addJsonObject {
                    put("id", c.id)
                    put("title", c.title)
                    put("severity", if (c.severity == AraSynthesisService.ConflictSeverity.BLOCK) "block" else "warn")
    ...
    fun resolveConflict(conflictId: String, resolution: String) {
        val choice = when (resolution) {
            "proceed" -> AraSynthesisService.ConflictResolutionChoice.PROCEED
            "defer"   -> AraSynthesisService.ConflictResolutionChoice.DEFER
            "reroute" -> AraSynthesisService.ConflictResolutionChoice.REROUTE
            else      -> AraSynthesisService.ConflictResolutionChoice.DEFER
        }
    ...
    private fun parseConflictJson(json: String): AraSynthesisService.ConflictResult? {
        return try {
            ...
            val severity = if (obj["severity"]?.jsonPrimitive?.contentOrNull == "block")
                AraSynthesisService.ConflictSeverity.BLOCK
            else
                AraSynthesisService.ConflictSeverity.WARN
            ...
            AraSynthesisService.ConflictResult(
                id = id,
                title = title,
