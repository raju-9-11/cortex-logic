 ...
 * Mirrors the web app's AraDataLayer for platform-agnostic storage.
 */
interface AraDataLayer {
    ...
    suspend fun <T> query(query: AraQuery, serializer: (String) -> T): List<T>
    fun <T> subscribeToQuery(query: AraQuery, serializer: (String) -> T, listener: (List<T>) -> Unit): () -> Unit
    suspend fun batchWrite(operations: List<BatchOperation>)
}

data class AraQueryFilter(
    val field: String,
    val op: String,
    val value: Any?
)

data class AraOrderBy(
    val field: String,
...
data class AraQuery(
    ...
    val where: List<AraQueryFilter> = emptyList(),
    val orderBy: List<AraOrderBy> = emptyList(),
    val limit: Int? = null
)
