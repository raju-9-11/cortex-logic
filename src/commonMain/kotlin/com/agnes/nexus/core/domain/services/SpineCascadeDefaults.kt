 ...
 * Uses NexusSettings as the localStorage equivalent when available.
 */
object SpineCascadeDefaults {
    ...
    fun register(eventBus: SpineEventBus, settings: NexusSettings? = null) {
    ...
    fun registerAtlasCascadeRules(eventBus: SpineEventBus, settings: NexusSettings? = null) {
    ...
    private fun isWeeklyReviewDue(settings: NexusSettings?): Boolean {
    ...
    private fun weeklyReviewRules(settings: NexusSettings?): List<CascadeRule> = listOf(
        CascadeRule(
    ...
    private fun isMorningBriefDue(settings: NexusSettings?): Boolean {
        val now = Clock.System.now()
        val local = now.toLocalDateTime(TimeZone.currentSystemDefault())
        val hour = local.hour
    ...
    private fun isEveningCheckinDue(settings: NexusSettings?): Boolean {
        val now = Clock.System.now()
        val local = now.toLocalDateTime(TimeZone.currentSystemDefault())
        val hour = local.hour
    ...
    private fun ATLAS_MORNING_RULES(settings: NexusSettings?): List<CascadeRule> = listOf(
        CascadeRule(
