...
import com.agnes.ara.core.domain.models.ModuleSchemaDefinition
...
class ActionHubTest {
    ...
    /**
     * In-memory AraDataLayer. Stores each document as a JSON string so that
     ...
     */
    class FakeAraDataLayer : AraDataLayer {
    ...
    private fun hub(dataLayer: FakeAraDataLayer? = null): Triple<ActionHub, FakeNeuralProjectionService, FakeSpineEventBus> {
        val nsv = FakeNeuralProjectionService()
        val bus = FakeSpineEventBus()
        return Triple(ActionHub(nsv, bus, dataLayer), nsv, bus)
    }

    private fun FakeAraDataLayer.prefillLedger(uid: String, profile: LedgerProfile) =
        prefill("ledger_profiles", uid, profile, json, LedgerProfile.serializer())

    private fun FakeAraDataLayer.prefillAtlas(uid: String, profile: AtlasProfile) =
        prefill("atlas_profiles", uid, profile, json, AtlasProfile.serializer())

    private fun FakeAraDataLayer.loadedLedger(uid: String): LedgerProfile {
    ...
    private fun FakeAraDataLayer.loadedAtlas(uid: String): AtlasProfile {
    ...
    fun updateBudget_patchesExistingCategory() = runTest {
        ...
        dl.prefillLedger("u1", LedgerProfile(
            budgetCategories = listOf(
                com.agnes.ara.core.domain.models.LedgerBudgetCategory(id = "b1", name = "Food", allocated = 300.0)
            )
        ))
        val (actionHub, _, bus) = hub(dl)
    ...
    fun journalEntryCreated_capsAtFiveHundredEntries() = runTest {
        ...
        val existingEntries = (1..500).map {
            com.agnes.ara.core.domain.models.AtlasJournalEntry(id = "e$it", freeText = "old")
        }
